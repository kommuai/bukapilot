...
class NavRouteTransfer:
  ...
  def expire(self, now: float | None = None) -> dict[str, Any] | None:
    ...
    if now is None:
      now = monotonic()
    idle = now - current["updatedAt"]
    if idle < ROUTE_TRANSFER_IDLE_TIMEOUT_SECONDS:
      return None
    expired = {key: current[key] for key in (
      "requestId", "transferId", "receivedParts", "partCount", "receivedBytes", "totalBytes",
    )}
    expired["idleSeconds"] = idle
    self._transfer = None
    return expired
