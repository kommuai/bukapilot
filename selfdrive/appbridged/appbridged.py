...
from openpilot.selfdrive.nav.destination_store import (
  NAV_LAST_OPERATION_KEY, NAV_OPERATION_RESULT_KEY, NAV_ROUTE_FAILURE_KEY, NAV_ROUTE_REQUEST_KEY,
...
class AppBridge:
  ...
  def _nav_ble_sync(self, out: dict, *, include_route_request: bool = True,
                    include_stop_snapshot: bool = True) -> None:
    """Send canonical nav state on Settings and only HUD-safe state on Visualisation."""
    try:
      ...
      with nav_stop_list_lock():
        ...
        if include_stop_snapshot:
          ...
          operation = _nav_json_param(NAV_OPERATION_RESULT_KEY, None)
          out['navOperationResult'] = operation if isinstance(operation, dict) else None
        if include_route_request:
  ...
  def send_visualisation_message(self, is_metric):
    ...
    self._nav_ble_sync(data, include_route_request=False, include_stop_snapshot=False)
    try:
  ...
  def _apply_nav_stop_operation(self, command: str, settings: dict) -> None:
    ...
    with nav_stop_list_lock():
      ...
      previous_result = _nav_json_param(NAV_OPERATION_RESULT_KEY, None)
      if operation_id == last_operation_id:
        if isinstance(previous_result, dict) and previous_result.get('operationId') == operation_id:
          cloudlog.warning(f'appbridged nav operation duplicate command={command} operation={operation_id}')
          return
        cloudlog.error(f'appbridged nav operation has no result command={command} operation={operation_id}')
        return
      ...
      result = {
        ...
        'activeStopChanged': active_stop_changed,
      }
      params.put(NAV_OPERATION_RESULT_KEY, json.dumps(result, separators=(',', ':')))
      params.put(NAV_LAST_OPERATION_KEY, operation_id)
      cloudlog.warning(
        f'appbridged nav operation command={command} operation={operation_id} ok={int(ok)} '
        f'reason={reason} count={result["count"]} version={result["version"]} '
  ...
  def _apply_nav_route_response_start(self, settings: dict, is_offroad: bool) -> None:
    ...
    with nav_stop_list_lock():
      ...
      request['transferUpdatedAtMonotonic'] = result['updatedAt']
  ...
  def _update_nav_ble_diag_progress(self, now):
    ...
    if progress is None:
      ...
      return
    if self._nav_ble_diag_stop_index == 1 and self._nav_ble_diag_route.arrived(progress, 0.0):
  ...
  def _maybe_send_nav_ble_diag_mapbox_probe(self, test_id, now):
    ...
    if self._nav_ble_diag_mapbox_interval_sec <= 0.0 or self._nav_ble_diag_max_mapbox_probes <= 0:
      return
    sequence = int(elapsed // self._nav_ble_diag_mapbox_interval_sec) + 1
  ...
  def _apply_nav_ble_diag_skip_stop(self, settings, is_offroad):
    ...
    if not isinstance(test_id, str) or test_id != self._nav_ble_diag_test_id:
      ...
      stop_name = self._nav_ble_diag_route_stops[1][0]
      self._set_nav_ble_diag_status('waiting_for_route', f'Waiting for directions to {stop_name}')
      accepted = True
      reason = 'skipped'
