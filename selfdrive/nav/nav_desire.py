...
def navigation_suppresses_alc(carstate, lateral_active: bool) -> bool:
  ...
  if modifier in ("left", "sharpLeft", "right", "sharpRight"):
    turn_left = "left" in modifier.lower()
    same_side_blinker = carstate.leftBlinker if turn_left else carstate.rightBlinker
    opposite_blinker = carstate.rightBlinker if turn_left else carstate.leftBlinker
    blindspot = carstate.leftBlindspot if turn_left else carstate.rightBlindspot
    if (not same_side_blinker or opposite_blinker or blindspot or
        carstate.standstill or carstate.vEgo >= LANE_CHANGE_SPEED_MIN):
      return False
