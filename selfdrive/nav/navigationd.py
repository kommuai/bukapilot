...
class Navigationd:
  ...
  def _maybe_request_route_locked(self, dest: dict | None, location_valid: bool, now: float, *, reason: str) -> None:
    ...
    if raw:
      ...
      if request_id and (
        ...
        self._clear_route_request('stale_destination_or_session')
        raw = ''
        request_id = ''
      try:
  ...
  def _maybe_abandon(self, dest: dict | None, location_valid: bool, route_state: dict | None, progress: RouteProgress | None) -> None:
    """Advance the device stop queue only after clearly leaving the destination."""
    if dest is None or self._dest_key(dest) != self._last_dest_key or not location_valid or self._last_position is None:
      self._abandon_started_at = None
      return
    try:
    ...
    if self._timer_expired(self._abandon_started_at, ABANDON_HOLD_SECONDS, now):
      cloudlog.warning(f"navigationd abandon destination key={self._dest_key(dest)}")
      if not self._advance_stop(self._dest_key(dest), self._route_session_id, reason='abandon'):
        self._abandon_started_at = None
