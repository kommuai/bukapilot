...
def _same_stop(first: dict[str, Any], second: dict[str, Any]) -> bool:
  return abs(first["latitude"] - second["latitude"]) < 1e-5 and abs(first["longitude"] - second["longitude"]) < 1e-5
...
def write_stop_list(params: Any, stops: Any, *, index: int = 0) -> list[dict[str, Any]]:
  ...
  params.put(NAV_STOP_INDEX_KEY, max(0, min(_coerce_int(index), len(normalized))))
  params.put(NAV_STOP_VERSION_KEY, _coerce_int(params.get(NAV_STOP_VERSION_KEY)) + 1)
  if normalized:
...
def apply_stop_operation(stops: Any, command: str, payload: dict[str, Any]) -> tuple[list[dict[str, Any]], bool, str, bool]:
  ...
  if command == "navReorderStops":
    ...
    if any(_same_stop(first, second) for first, second in zip(updated, updated[1:])):
      return current, False, "adjacent_duplicate", False
