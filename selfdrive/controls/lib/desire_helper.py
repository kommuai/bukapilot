...
class DesireHelper:
  ...
  def update(self, carstate, lateral_active, lane_change_prob, md=None):
    ...
    nav_changed = int(nav_took_over and final_name != base_name)
    if nav_took_over:
      source = "nav"
    ...
    log_key = (
      ...
      int(nav_took_over), nav_changed, int(one_blinker), self.lane_change_state,
    ...
    if log_key != self._last_final_desire_log_key and (
      ...
      cloudlog.warning(
        ...
        f"nav_took_over={int(nav_took_over)} nav_changed={nav_changed} "
        f"blinker={int(one_blinker)} "
